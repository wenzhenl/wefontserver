...
if(!$stmt4_result) rollback_and_exit($conn, 'DB op failure: unable to perform SELECT');
if(mysqli_num_rows($stmt4_result) == 0) rollback_and_exit($conn, 'no result returned from SELECT');
...
if(!file_put_contents($image_path, $user_image_data)) 
    rollback_and_exit($conn, 'failed to write glyph image at path '.$image_path);
...
if(is_null($output)) rollback_and_exit($conn, 'fails to add glyph into font');
