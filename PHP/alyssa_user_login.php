...
if (empty($user_email) || empty($user_psw)) exit_with_error('JSON object error');

$conn = connect_AlyssaDB();
$user_email = mysqli_real_escape_string($conn, $user_email);
$user_psw = mysqli_real_escape_string($conn, $user_psw);

$stmt = "SELECT user_id, user_nickname, user_password FROM User WHERE user_email = '$user_email' ";
$result = exec_query ($conn, $stmt);

if (mysqli_num_rows($result) == 0) exit_with_error('user email does not exist');
...
if(!password_verify($user_psw, $user_psw_encoded)) exit_with_error('user password incorrect');
...
if (mysqli_num_rows($result) == 0) {
    // $return_data['message'] =' No active font found';
