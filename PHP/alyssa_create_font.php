...
function create_font_dir ($user_id, $user_font_id) {
...
function exec_query_and_create_dir($conn, $user_id, $user_fontname){
    ...
    $stmt = "SELECT fond_id FROM Font WHERE user_id = '$user_id' AND fontname = '$user_fontname' ";
    ...
    if (create_font_dir($user_id, $user_font_id)) {
        mysqli_commit($conn);
